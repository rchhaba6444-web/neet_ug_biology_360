...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'kavirani576@gmail.com';
...
app.post('/api/admin/upload', authenticateAdmin, upload.single('file'), async (req, res) => {
  try {
    ...
    if (type === 'mocktest' && testId) {
      result = await db.run(
        'UPDATE content SET file_url = ?, file_name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND type = "mocktest"',
        [file.path, file.originalname, testId]
      );
    } else if (type === 'diagram' && diagramId) {
      result = await db.run(
        'UPDATE content SET file_url = ?, file_name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND type = "diagrams"',
        [file.path, file.originalname, diagramId]
      );
    } else {
      // Insert new content
      result = await db.run(
        'INSERT INTO content (title, type, file_url, file_name, is_premium, created_at) VALUES (?, ?, ?, ?, 1, CURRENT_TIMESTAMP)',
        [file.originalname, type, file.path, file.originalname]
      );
    }
    ...
    if (result.changes > 0) {
      res.json({
        ...
        fileId: result.lastID || (testId || diagramId),
        fileName: file.originalname,
        filePath: file.path
      });
    } else {
      res.status(400).json({ error: 'Failed to save file information' });
    }
  } catch (error) {
